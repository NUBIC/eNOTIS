module PublicSurveyorHelper
  ...
  def previous_section
    ...
    submit_tag(t('surveyor.previous_section'), :name => "section[#{@sections[@sections.index(@section)-1].id}]",:class=>"awesome regular") unless @sections.first == @section
  end
  def next_section
    # use copy in memory instead of making extra db calls
    @sections.last == @section ? submit_tag(t('surveyor.click_here_to_finish'), :name => "finish",:class=>"awesome regular") : submit_tag(t('surveyor.next_section'), :name => "section[#{@sections[@sections.index(@section)+1].id}]",:class=>"awesome regular")
  end
