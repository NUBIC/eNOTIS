module Authentication
  mattr_accessor :netid_regex, :bad_netid_message, 
    :name_regex, :bad_name_message,
    :email_name_regex, :domain_head_regex, :domain_tld_regex, :email_regex, :bad_email_message

  self.netid_regex       = /\A\w[\w\.\-_@]+\z/                     # ASCII, strict
  # self.netid_regex       = /\A[[:alnum:]][[:alnum:]\.\-_@]+\z/     # Unicode, strict
  # self.netid_regex       = /\A[^[:cntrl:]\\<>\/&]*\z/              # Unicode, permissive

  self.bad_netid_message = "use only letters, numbers, and .-_@ please.".freeze
