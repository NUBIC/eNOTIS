...
describe ExternalEventProcessor do 
  ...
  it "should process an event properly when it receives one" do
    params = {:study=>{:irb_number=>@study.irb_number}, 
      :subject=>{:mrn=>'1234', :mrn_type=>"NMH",:birth_date=>"1988-11-02", :last_name=>"kabaka", :middle_name=>"man",:first_name=>"daudi"}, 
      :involvement=>{:case_number=>"12345",:gender=>"male", :race_is_asian=>"T", :ethnicity=>"Hispanic Or Latino"}, 
      :involvement_events=>[{:occurred_on=>"2010-11-18", :note=>"generated from Registar - Asthma and COPD Patient Registry", :event=>"consented"}]}
    ...
    e = involvement.involvement_events.first
    e.event.should =="Consented"
    e.occurred_on.to_date.should == Chronic.parse("2010-11-18").to_date
  end
