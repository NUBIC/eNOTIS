...
describe SubjectsController do
  describe "importing csv" do
    ...
    it "should allow me to post the file to create" do
      #controller.stub!(:csv_sanity_check).and_return(true)
      #post :create, {:file => @good_csv}
      #response.should redirect_to(studies_path)
      #post :create, {:file => @good_csv, :study_id => 'STU00002629'}
      #response.should redirect_to(study_path(:id => 'STU00002629'))
    end
  
    it "should create a new StudyUpload with the file attached" do
      #controller.stub!(:csv_sanity_check).and_return(true)
      #StudyUpload.should_receive(:create).with({:upload => @good_csv, :study_id => "'STU00002629'",:user_id=>1}).and_return(Factory(:study_upload))
      #post :create, {:file => @good_csv, :study_id => 'STU00002629'}
    end
    it "should sanity check csv file for mrn or (first_name, last_name, dob), and (subject_event_type, subject_event_date)" do
      #controller.csv_sanity_check(@good_csv).should be_true
      #controller.csv_sanity_check(@bad_csv).should be_false
    end
    it "should sanity check csv file for mrn or (first_name, last_name, dob), and (subject_event_type, subject_event_date) with columns in random order" do
      #controller.csv_sanity_check(@good_random_cols_csv).should be_true
      #controller.csv_sanity_check(@bad_random_cols_csv).should be_false
    end
    it "should sanityf check a csv file or string or Paperclip::Attachement" do
      #controller.csv_sanity_check(@bad_csv).should be_false
      #controller.csv_sanity_check(@good_csv.read.to_s).should be_true
      #controller.csv_sanity_check(StudyUpload.new(:upload => @bad_random_cols_csv).upload).should be_false
    end
    it "should sanity check csv file (success) and queue up the file and redirect to study" do
      #controller.should_receive(:csv_sanity_check).and_return(true)
      #post :create, {:file => @good_csv, :study_id => 'STU00002629'}
      #response.should redirect_to(study_path(:id => 'STU00002629'))
    end
    ...
    it "should sanity check csv file (failure) and send me back a csv file" do
      #controller.should_receive(:csv_sanity_check).and_return(false)
      #post :create, {:file => @bad_csv, :study_id => 'STU00002629'}
